...
@ServerEndpoint(
        ...
        port = "8989",
        host = "0.0.0.0",
        maxFramePayloadLength = "6553600",
        allIdleTimeSeconds = "300")
public class NettyWebSocket {

    /**
     * concurrent包的线程安全Set，用来存放每个客户端对应的MyWebSocket对象。
     */
    private static ConcurrentHashMap<String, NettyWebSocket> userIdWebSocketMap =
    ...
    public void onOpen(
        ...
        LogUtils.debug(
                "UserId = {}, 通道ID={}, 当前连接人数={}", "xxx", getSessionId(session), getOnlineCount());
    }
    ...
    public void onClose(Session session) throws IOException {
        ...
        if (userId != null) {
            ...
            LogUtils.debug(
                    "==============>>>>>>>>>>>>>>>{},用户退出,当前在线人数为:{}", userId, getOnlineCount());
        }
    }
    ...
    public void onError(Session session, Throwable cause) {
        ...
        if (userId != null) {
            if (Objects.nonNull(cause) && !(cause instanceof EOFException)) {
                LogUtils.error(
                        "UserId = {}, 通道ID={}, 出错信息={}", userId, session.id(), cause.toString());
            }
            if (session.isOpen()) {
    ...
    public void onMessage(Session session, String message) {
        LogUtils.debug(
                "用户消息:{},报文:{},session现有的主题：{},主题：{}",
                session.getAttribute("userId"),
                message,
                session.getAttribute("F39_PAN_KOU"),
                session.getAttribute("F39_PAN_KOU_GCC"));
    ...
    //		} catch (IOException e) {
    //			LogUtils.error("UserId = {}, 通道ID={}, 解析执行出错信息={}", userInfo.toString(),
    // getSessionId(session), e.getMessage());
    //		}
    //	}
    ...
    private String checkToken(HttpHeaders headers, Session session) {
        ...
        String redisToken =
                (String) redisRepository.get(StringUtils.join("netty-websocket-online-", userId));
    ...
    public static void sendInfo(Chat chat, List<Chat> chats) {
        try {
            if (!StringUtils.isEmpty(chat.getTargetUserId().toString())
                    && userIdWebSocketMap.containsKey(chat.getTargetUserId().toString())) {
                //
                //	userIdWebSocketMap.get(chat.getUserId().toString()).sendMessage(JSONObject.toJSONString(chats));
                userIdWebSocketMap
                        .get(chat.getTargetUserId().toString())
                        .sendMessage(JSONObject.toJSONString(chats));
            } else {
                userIdWebSocketMap
                        .get(chat.getUserId().toString())
                        .sendMessage(JSONObject.toJSONString(chats));
            }
        } catch (IOException e) {
            LogUtils.error(e);
