#!/bin/bash
# taotao-cloud-message 安全扫描脚本
# 用于检查 DDD 项目中的常见安全问题

echo "================================================"
echo "  taotao-cloud-message 安全扫描"
echo "================================================"

# 扫描配置
BASE_DIR="."
MODULES=("taotao-cloud-message-api" "taotao-cloud-message-application" "taotao-cloud-message-domain" "taotao-cloud-message-infrastructure" "taotao-cloud-message-interfaces")
HAS_WARNINGS=0
...
for module in "${MODULES[@]}"; do
    if [ -d "$BASE_DIR/$module/src/main/java" ]; then
        found=$(grep -rn "\.save\|\.update\|\.delete\|\.findById\|@Table\|@Entity" \
            "$BASE_DIR/$module/src/main/java/com/taotao/cloud/message/domain" \
            --include="*.java" 2>/dev/null || true)
        if [ -n "$found" ]; then
